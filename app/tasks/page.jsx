              ...
              <tbody>
                {tasks.map((task) => (
                  <tr key={task._id} className="border-t border-gray-100 hover:bg-gray-50/50 transition">
                    <td className="px-4 py-3 text-gray-900 font-medium cursor-pointer hover:text-blue-600 transition" onClick={() => router.push(`/tasks/${task._id}`)}>{task.title}</td>
            ...
            {tasks.map((task) => (
              <div key={task._id} className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
                <div className="flex justify-between items-start gap-2">
                  <h3 className="text-sm font-semibold text-gray-900 cursor-pointer hover:text-blue-600 transition" onClick={() => router.push(`/tasks/${task._id}`)}>{task.title}</h3>
                  <select
                    value={task.status}
                    onChange={(e) => updateStatus(task._id, e.target.value)}
